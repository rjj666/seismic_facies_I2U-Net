...
class section_loader(data.Dataset):
    ...
    def __init__(self, split='train', is_transform=True,
        ...
        self.inline_sections = []  # 存储 inline 切片
        self.crossline_sections = []  # 存储 crossline 切片
        ...
        if 'test' not in self.split:
            ...
            for split in ['train', 'val', 'train_val']:
                ...
                # 分离 inline 和 crossline 切片
                for section in file_list:
                    if section.startswith('i_'):
                        self.inline_sections.append(section)
                    elif section.startswith('x_'):
                        self.crossline_sections.append(section)
        elif 'test' in split:
            # We are in test mode. Only read the given split. The other one might not 
            # be available. 
            path = pjoin('data', 'splits', 'section_' + split + '.txt')
    ...
    def __getitem__(self, index):
        ...
        if direction == 'i':
            im = self.seismic[int(number),:,:]  # [255, 701]
            lbl = self.labels[int(number),:,:]
        elif direction == 'x':    
            im = self.seismic[:,int(number),:]  # [255, 401]
            lbl = self.labels[:,int(number),:]
    ...
    def get_batch_indices(self, batch_size):
        """
        获取平衡的批次索引，确保每个批次包含相似数量的 inline 和 crossline 切片
        """
        inline_indices = np.random.choice(len(self.inline_sections), batch_size // 2, replace=False)
        crossline_indices = np.random.choice(len(self.crossline_sections), batch_size - batch_size // 2, replace=False)
        
        inline_sections = [self.sections[self.split].index(self.inline_sections[i]) for i in inline_indices]
        crossline_sections = [self.sections[self.split].index(self.crossline_sections[i]) for i in crossline_indices]
        
        return inline_sections + crossline_sections
    ...
    def transform(self, img, lbl):
        ...
        # 添加通道维度
        img = np.expand_dims(img, 0)  # 添加通道维度 [C, H, W]
        lbl = np.expand_dims(lbl, 0)  # 添加通道维度 [C, H, W]
...
class CustomSamplerTrain(data.Sampler):
    def __init__(self, data_list):
        self.data_list = data_list
        self.inline_indices = [i for i, x in enumerate(data_list) if x.startswith('i_')]
        self.crossline_indices = [i for i, x in enumerate(data_list) if x.startswith('x_')]
        
    def __iter__(self):
        # 分别打乱 inline 和 crossline 索引
        inline_indices = np.random.permutation(self.inline_indices)
        crossline_indices = np.random.permutation(self.crossline_indices)
        
        # 分组返回索引
        i, c = 0, 0
        while i < len(inline_indices) or c < len(crossline_indices):
            if i < len(inline_indices):
                # 返回一组 inline 索引
                group_size = min(8, len(inline_indices) - i)  # 假设批次大小为8
                yield from inline_indices[i:i+group_size]
                i += group_size
            
            if c < len(crossline_indices):
                # 返回一组 crossline 索引
                group_size = min(8, len(crossline_indices) - c)  # 假设批次大小为8
                yield from crossline_indices[c:c+group_size]
                c += group_size
        
    def __len__(self):
        return len(self.data_list)

class CustomSamplerVal(data.Sampler):
    def __init__(self, data_list):
        self.data_list = data_list
        self.inline_indices = [i for i, x in enumerate(data_list) if x.startswith('i_')]
        self.crossline_indices = [i for i, x in enumerate(data_list) if x.startswith('x_')]
    ...
    def __iter__(self):
        # 分别返回 inline 和 crossline 索引
        yield from self.inline_indices
        yield from self.crossline_indices
