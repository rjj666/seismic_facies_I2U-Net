...
class I2U_Net(nn.Module):
    """
    Modified I2U-Net for facies classification
    """
    def __init__(self, classes=6, channels=1):
        super(I2U_Net, self).__init__()
    ...
    def forward(self, x):
        ...
        # 打印各层特征图尺寸，用于调试
        # print(f"Input: {input_size}, x: {x.size()}, e1: {e1.size()}, e2: {e2.size()}, e3: {e3.size()}, e4: {e4.size()}")
        ...
        # 打印解码器特征图尺寸，用于调试
        # print(f"d4: {d4.size()}, d3: {d3.size()}, d2: {d2.size()}, d1: {d1.size()}")
